...
func CaGetChannels_pindao(params url.Values) dto.ReturnJsonDto {
	caId := params.Get("caId")
	println("caId：", caId)
	if caId == "" {
		return dto.ReturnJsonDto{Code: 0, Msg: "频道列表id为空", Type: "danger"}
	}

	var categoryDb models.IptvFenlei

	if err := dao.DB.Where("id = ?", caId).First(&categoryDb).Error; err != nil {
		return dto.ReturnJsonDto{Code: 0, Msg: "该分类不存在", Type: "danger"}
	}
	channels := until.CaGetChannels_pindao(categoryDb)

	var category []models.IptvCategory
	dao.DB.Table(models.IptvCategory{}.TableName() + " AS c").
		Select("c.*").
		Order("sort asc").
		Find(&category)
...
func UpdateInterval_Fenlei(params url.Values) dto.ReturnJsonDto {
...
func AddList_Fenlei(params url.Values) dto.ReturnJsonDto {
...
func UpdateList_Fenlei(params url.Values) dto.ReturnJsonDto {
...
func DelList_Fenlei(params url.Values) dto.ReturnJsonDto {
	...
	var iptvCategoryList models.IptvPindao
	res := dao.DB.Model(&models.IptvPindao{}).Where("id = ?", listId).First(&iptvCategoryList)

	if res.RowsAffected == 0 {
		return dto.ReturnJsonDto{Code: 0, Msg: "删除频道列表失败，频道列表不存在", Type: "danger"}
	}
	...
	dao.DB.Where("id = ?", iptvCategoryList.ID).Delete(&models.IptvPindao{})
...
func DelCa_Fenlei(params url.Values) dto.ReturnJsonDto {
	caId := params.Get("delca")
	if caId == "" {
		return dto.ReturnJsonDto{Code: 0, Msg: "参数错误", Type: "danger"}
	}

	var category models.IptvFenlei
	if err := dao.DB.Model(&models.IptvFenlei{}).Where("id = ?", caId).First(&category).Error; err != nil {
		return dto.ReturnJsonDto{Code: 0, Msg: "该分类不存在", Type: "danger"}
	}

	dao.DB.Model(&models.IptvFenlei{}).Where("id = ?", category.ID).Delete(&models.IptvFenlei{})
	dao.DB.Model(&models.IptvChannel{}).Where("c_id = ?", category.ID).Delete(&models.IptvChannel{})
	go until.CleanAutoCacheAll()
	return dto.ReturnJsonDto{Code: 1, Msg: fmt.Sprintf("删除分类 %s 成功\n", category.Name), Type: "success"}
}

func SubmitMoveUp_Fenlei(params url.Values) dto.ReturnJsonDto {
	id := params.Get("moveup")
	if id == "" {
		return dto.ReturnJsonDto{Code: 0, Msg: "参数错误", Type: "danger"}
	}
	var current, prev models.IptvFenlei
	print(id)
	if err := dao.DB.Model(&models.IptvFenlei{}).Where("id = ?", id).First(&current).Error; err != nil {
		return dto.ReturnJsonDto{Code: 0, Msg: "未找到当前记录", Type: "danger"}
	}
	if err := dao.DB.Model(&models.IptvFenlei{}).
	...
	err := dao.DB.Transaction(func(tx *gorm.DB) error {
		// 交换 sort
		if err := tx.Model(&models.IptvFenlei{}).
			Where("id = ?", current.ID).
...
func SubmitMoveDown_Fenlei(params url.Values) dto.ReturnJsonDto {
	id := params.Get("movedown")
	if id == "" {
		return dto.ReturnJsonDto{Code: 0, Msg: "参数错误", Type: "danger"}
	}

	var current, next models.IptvFenlei
...
func SubmitMoveTop_Fenlei(params url.Values) dto.ReturnJsonDto {
	id := params.Get("movetop")
	if id == "" {
		return dto.ReturnJsonDto{Code: 0, Msg: "参数错误", Type: "danger"}
	}

	var current models.IptvFenlei
...
func SubmitSave_Fenlei(params url.Values) dto.ReturnJsonDto {
...
// 保存频道
func SaveChannelsOne_Fenlei(params url.Values) dto.ReturnJsonDto {
	chId := params.Get("chId")
	chname := params.Get("chname")
	rss_rules := params.Get("rss_rules")
	epg_rules := params.Get("epg_rules")
	rss_ids := params["rss_list_id[]"]
	epg_ids := params["epg_list_id[]"]

	// 将切片中的元素用逗号连接
	r_id := strings.Join(rss_ids, ",")
	e_id := strings.Join(epg_ids, ",")
	fmt.Println("e_ids22：", r_id)

	println("SaveChannelsOne_Fenlei：", chId, chname, r_id, e_id)
	if chId == "" || chname == "" || r_id == "" || e_id == "" {
		return dto.ReturnJsonDto{Code: 0, Msg: "添加频道参数错误, 不得为空", Type: "danger"}
	}

	if !until.IsSafe(chId) || !until.IsSafe(r_id) || !until.IsSafe(e_id) {
	...
	var channel models.IptvPindao
	if err := dao.DB.Model(&models.IptvPindao{}).Where("id = ?", chId).First(&channel).Error; err != nil {
		return dto.ReturnJsonDto{Code: 0, Msg: "未找到对应的频道记录", Type: "danger"}
	}

	channel.Name = chname
	channel.Rss_Rules = rss_rules
	channel.Epg_Rules = epg_rules
	channel.EId = 0
	channel.Rss_List = r_id
	channel.Epg_List = e_id

	if err := dao.DB.Model(&models.IptvPindao{}).Where("id = ?", chId).Save(&channel).Error; err != nil {
	...
	return dto.ReturnJsonDto{Code: 1, Msg: "保存频道成功", Type: "success"}
}

func GenreChannels_Fenlei(listName, srclist string, listId int64, doRepeat bool) dto.ReturnJsonDto {
...
func CategoryListChangeStatus_Fenlei(params url.Values) dto.ReturnJsonDto {
...
func FenleiChangeStatus(params url.Values) dto.ReturnJsonDto {
	caId := params.Get("fenleiStatus")
	if caId == "" {
		return dto.ReturnJsonDto{Code: 0, Msg: "源 id 不能为空", Type: "danger"}
	}

	var cateData models.IptvFenlei
	if err := dao.DB.Model(&models.IptvFenlei{}).Where("id = ?", caId).First(&cateData).Error; err != nil {
		return dto.ReturnJsonDto{Code: 0, Msg: "查询fenlei失败", Type: "danger"}
	}

	if cateData.Enable == 1 {
		dao.DB.Model(&models.IptvFenlei{}).Where("id = ?", cateData.ID).Update("enable", 0)
		//dao.DB.Model(&models.IptvChannel{}).Where("c_id = ?", cateData.ID).Update("status", 0)
	} else {
		dao.DB.Model(&models.IptvFenlei{}).Where("id = ?", cateData.ID).Update("enable", 1)
...
// 前段修改频道状态
func ChannelsChangeStatus_Fenlei(params url.Values) dto.ReturnJsonDto {
	chId := params.Get("channelsStatus")
	if chId == "" {
		return dto.ReturnJsonDto{Code: 0, Msg: "频道 id 不能为空", Type: "danger"}
	}

	var chData models.IptvPindao
	if err := dao.DB.Model(&models.IptvPindao{}).Where("id = ?", chId).First(&chData).Error; err != nil {
		return dto.ReturnJsonDto{Code: 0, Msg: "查询频道失败11", Type: "danger"}
	}

	if chData.Status == 1 {
		dao.DB.Model(&models.IptvPindao{}).Where("id = ?", chData.ID).Update("status", 0)
	} else {
		dao.DB.Model(&models.IptvPindao{}).Where("id = ?", chData.ID).Update("status", 1)
	}
	go until.CleanAutoCacheAll()
	return dto.ReturnJsonDto{Code: 1, Msg: "频道 " + chData.Name + "状态修改成功", Type: "success"}
}

func UpdateListAll_Fenlei() dto.ReturnJsonDto {
...
func UploadPayList_Fenlei(c *gin.Context) dto.ReturnJsonDto {
...
// 前段保存分类
func SaveFenlei(params url.Values) dto.ReturnJsonDto {
	caId := params.Get("caId")
	...
	if caId == "" {
		var tmpCa models.IptvFenlei
		err := dao.DB.Model(&models.IptvFenlei{}).Where("name = ?", caname).First(&tmpCa).Error
		if err == nil {
			// 找到记录 → 重复
		...
		var maxSort int64
		dao.DB.Model(&models.IptvFenlei{}).Select("IFNULL(MAX(sort),0)").Scan(&maxSort)
		var new = models.IptvFenlei{Name: caname, Type: "user", Sort: maxSort + 1, UA: caua}
		...
		dao.DB.Model(&models.IptvFenlei{}).Create(&new)
		if new.Type == "auto" {
			go until.CleanAutoCacheAll()
		}
	} else {
		...
		dao.DB.Model(&models.IptvFenlei{}).Where("id = ?", caIdInt).Updates(&ca)
		if ca.Type == "auto" {
			go until.CleanAutoCacheAll()
		}
	}
	return dto.ReturnJsonDto{Code: 1, Msg: "分类操作成功", Type: "success"}
}
func AddPindao(params url.Values) dto.ReturnJsonDto {
	//listName := params.Get("listname")
	pindao_name := params.Get("pindao_name")
	//url := strings.TrimSpace(params.Get("listurl"))
	//ua := params.Get("listua")
	clId := params.Get("caId")
	autocategory := params.Get("autocategory")
	//repeat := params.Get("repeat")
	println("添加频道名称:", pindao_name)
	println("caId:", clId)
	if pindao_name == "" {
		return dto.ReturnJsonDto{Code: 0, Msg: "请输入频道名称", Type: "danger"}
	}

	if !until.IsSafe(pindao_name) || !until.IsSafe(autocategory) || !until.IsSafe(clId) {
		return dto.ReturnJsonDto{Code: 0, Msg: "输入不合法", Type: "danger"}
	}

	// 转换为 int64
	id, err := strconv.ParseInt(clId, 10, 64) // 参数：字符串, 十进制, 64位
	if err != nil {
		// 转换失败处理（如返回错误响应）
		return dto.ReturnJsonDto{Code: 0, Msg: "参数必须是有效数字", Type: "danger"}
	}

	var channel models.IptvChannel
	channel.Name = pindao_name
	channel.Url = "123123"
	channel.EId = 1
	channel.CId = id
	channel.Status = 1

	if err := dao.DB.Model(&models.IptvChannel{}).Where("id = ?", clId).Save(&channel).Error; err != nil {
		return dto.ReturnJsonDto{Code: 0, Msg: "保存频道失败" + err.Error(), Type: "danger"}
	}
